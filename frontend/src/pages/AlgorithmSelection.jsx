...
const ALGORITHMS = [
  {
    ...
    description: 'Processes are executed in the order they arrive. Simple and fair, but can cause convoy effect.',
    color: '#6366f1'
  },
  {
    id: 'SJF',
    name: 'Shortest Job First',
    description: 'Selects the process with the shortest burst time. Optimal for minimizing average waiting time.',
    color: '#22c55e'
  },
  {
    id: 'LJF',
    name: 'Longest Job First',
    description: 'Selects the process with the longest burst time. Can cause starvation for short processes.',
    color: '#f59e0b'
  },
  {
    id: 'Round Robin',
    name: 'Round Robin',
    description: 'Each process gets a fixed time quantum. Fair and bounded response time, but depends on quantum size.',
    color: '#f43f5e'
  }
];
...
function AlgorithmSelection() {
  ...
  const handleSubmit = async () => {
    ...
    if (selected.includes('Round Robin') && (timeQuantum < 1 || !Number.isInteger(Number(timeQuantum)))) {
    ...
    <div className="algorithm-page">
      ...
      <div className="actions">
        ...
        <button className="btn btn-primary" onClick={handleSubmit} disabled={loading}>
          {loading ? 'Running Algorithms...' : `Run ${selected.length} Algorithm${selected.length !== 1 ? 's' : ''}`}
        </button>
      </div>
    </div>
  );
